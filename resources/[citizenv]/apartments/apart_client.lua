...
function MenuAppartement()
    ...
    for i=1, #appart do
        if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), appart[i].xe,appart[i].ye,appart[i].ze, true) < distance then
            DrawMarker(1,appart[i].xe,appart[i].ye,appart[i].ze-1.0001, 0, 0, 0, 0, 0, 0, 1.01, 1.01, 0.3, 212, 189, 0, 105, 0, 0, 2, 0, 0, 0, 0)
            if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), appart[i].xe,appart[i].ye,appart[i].ze, true) < 1.599 then
                TriggerServerEvent("apart:getAppart", appart[i].name)
                Wait(250)
                if isBuy == 1 then
...
function Vendre()
	for i=1, #appart do
        if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), appart[i].xe,appart[i].ye,appart[i].ze, true) < distance then
            DrawMarker(1,appart[i].xe,appart[i].ye,appart[i].ze-1.0001, 0, 0, 0, 0, 0, 0, 1.01, 1.01, 0.3, 212, 189, 0, 105, 0, 0, 2, 0, 0, 0, 0)
            if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), appart[i].xe,appart[i].ye,appart[i].ze, true) < 1.599 then
                TriggerServerEvent("apart:sellAppart", appart[i].name, appart[i].price)
                CloseMenu()
            end
        end
    end
...
function Acheter()
	...
	for i=1, #appart do
        if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), appart[i].xe,appart[i].ye,appart[i].ze, true) < distance then
            DrawMarker(1,appart[i].xe,appart[i].ye,appart[i].ze-1.0001, 0, 0, 0, 0, 0, 0, 1.01, 1.01, 0.3, 212, 189, 0, 105, 0, 0, 2, 0, 0, 0, 0)
            if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), appart[i].xe,appart[i].ye,appart[i].ze, true) < 1.599 then
                TriggerServerEvent("apart:buyAppart", appart[i].name, appart[i].price)
                CloseMenu()
            end
        end
    end
        ...
        if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), appart[i].xe,appart[i].ye,appart[i].ze, true) < distance then
            ...
            if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), appart[i].xe,appart[i].ye,appart[i].ze, true) < 1.599 then
                if timer == 0 then
                    ...
                    Wait(1000)
                    SetEntityCoords(GetPlayerPed(-1), appart[i].xo,appart[i].yo,appart[i].zo)
                    SetEntityHeading(GetPlayerPed(-1), appart[i].ho)
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(0)
        if timer > 0 and current_intappart > 0 then DrawAdvancedText(0.707, 0.77, 0.005, 0.0028, 1.89, "~b~"..appart[current_intappart].name, 255, 255, 255, 255, 1, 1) end
            for i=1, #appart do
                if not IsEntityDead(PlayerPedId()) then
                    ...
                    if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), appart[i].xo,appart[i].yo,appart[i].zo, true) < distance then
                        DrawMarker(1,appart[i].xo,appart[i].yo,appart[i].zo-1.0001, 0, 0, 0, 0, 0, 0, 1.01, 1.01, 0.3, 212, 189, 0, 105, 0, 0, 2, 0, 0, 0, 0)
                        if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), appart[i].xo,appart[i].yo,appart[i].zo, true) < 1.599 then
                            if timer == 0 then
                                DoScreenFadeOut(1000)
                                while IsScreenFadingOut() do Citizen.Wait(0) end
                                NetworkFadeOutEntity(GetPlayerPed(-1), true, false)
                                Wait(1000)
                                SetEntityCoords(GetPlayerPed(-1), appart[i].xe,appart[i].ye,appart[i].ze)
                                SetEntityHeading(GetPlayerPed(-1), appart[i].ho)
                                NetworkFadeInEntity(GetPlayerPed(-1), 0)
                                Wait(1000)
                                current_intappart = i
...
Citizen.CreateThread(function()
    while true do
        ...
        if timer > 0 then
            timer=timer-1
            if timer == 0 then current_intappart = 0 end
        end
    end
end)
