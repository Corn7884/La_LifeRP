  ...
  TriggerEvent('es:getPlayerFromId', source, function(user)
    ...
    if (tonumber(user.money) >= tonumber(price)) then
      ...
      elseif mode == "MySQL" then
    	...
    	TriggerClientEvent("citizenv:notify", source, "CHAR_SIMEON", 1, "Stephane", false, txt[lang]['welcome'])
    	TriggerClientEvent('apart:isMine', source)
    else
    	TriggerClientEvent("citizenv:notify", source, "CHAR_SIMEON", 1, "Stephane", false, txt[lang]['nocash'])
    end
  end)
end)
...
AddEventHandler('apart:sellAppart', function(name, price)
  TriggerEvent('es:getPlayerFromId', source, function(user)
    ...
    user:addMoney((price))
      ...
      TriggerClientEvent("citizenv:notify", source, "CHAR_SIMEON", 1, "Stephane", false, txt[lang]['estVendu'])
      TriggerClientEvent('apart:isNotBuy', source)
  end)
end)
