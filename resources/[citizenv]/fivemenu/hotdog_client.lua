...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(0)
		if (IsNearPoints(hotdog, 1.01) == true)  then
			if IsControlJustPressed(1, 38)  then
				TriggerServerEvent("inventory:checkMoneyDistrib", 100)
				Wait(200)
				if MoneyOk then
					TriggerEvent("food:veat", 40)
					MoneyOk = false
				end
				Wait(2000)
			end
		...
		if (IsNearPoints(hotdog, 1.01) == true)  then
			Info('Appuyer sur ~g~E~s~ pour manger un hotdog', false)
		end
	end
end)
