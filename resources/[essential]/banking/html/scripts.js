...
$(document).ready(function(){
  ...
  window.addEventListener('message', function(event){
    ...
    if(item.adddBalance == true) {
      var element = $('<p id="add-dbalance"><span class="pre">+</span><span class="green"> $ </span>' +addGaps(event.data.damount)+'</p>');
      $(".dtransaction").append(element);
    ...
    if(item.removedBalance == true) {
      var element = $('<p id="add-dbalance"><span class="pre">-</span><span class="red"> $ </span>' +addGaps(event.data.damount)+'</p>');
      $(".dtransaction").append(element);
      setTimeout(function(){
        $(element).fadeOut(600, function() { $(this).remove(); })
      }, 1000)
